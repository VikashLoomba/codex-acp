...
describe('ACP server test', { timeout: 40_000 }, () => {
    ...
    it('waits for typed mcp startup status updates and returns terminal states', async () => {
        ...
        mockFixture.sendServerNotification({
            ...
            params: { threadId: null, name: "alpha", status: "starting", error: null }
        });
        mockFixture.sendServerNotification({
            method: "mcpServer/startupStatus/updated",
            params: { threadId: null, name: "beta", status: "starting", error: null }
        });
        mockFixture.sendServerNotification({
            method: "mcpServer/startupStatus/updated",
            params: { threadId: null, name: "alpha", status: "ready", error: null }
        });
        mockFixture.sendServerNotification({
            method: "mcpServer/startupStatus/updated",
            params: { threadId: null, name: "beta", status: "ready", error: null }
        });
